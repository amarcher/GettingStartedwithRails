...
RSpec.describe Article, :type => :model do
  it "will create a new article in the db with provided title and text" do
    article = Article.new(title: "You are my sunshine", text: "My only sunshine")
    expect { article.save! }.to change{ Article.count }.by 1
  end

  it "will raise an ActiveRecord::RecordInvalid error trying to create an article w/o a title" do
    expect { Article.create!(text: "My only sunshine") }.to raise_error ActiveRecord::RecordInvalid
  end

  it "will raise an ActiveRecord::RecordInvalid error trying to create an article w/ a title of less than five characters" do
    expect { Article.create!(title: "You", text: "My only sunshine") }.to raise_error ActiveRecord::RecordInvalid
  end

  it "will *not* raise an error trying to create an article without text" do
    expect { Article.create!(title: "You are my sunshine") }.not_to raise_error
  end

  describe '#title' do
    it 'should return a string (eq to the article\'s title)' do
  ...
  describe '#text' do
    it 'should return a string (eq to the article\'s text)' do
  ...
  describe '#comments' do
    it 'should respond to the method #comments' do
      article = Article.create!(title: "You are my sunshine", text: "My only sunshine")
